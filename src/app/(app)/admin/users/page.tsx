...
export default async function AdminUsersPage() {
  ...
  const initialRows = mapProfileRows(data ?? null);
      ...
      <div className="relative flex min-h-0 flex-1 flex-col px-4 pb-10 pt-8 sm:px-6 lg:max-w-[90rem] lg:px-6 lg:pb-8 lg:pt-6 xl:mx-auto xl:w-full xl:px-8">
        <header className="mb-8 shrink-0">
          ...
          <p className="mt-2 max-w-3xl text-sm text-stone-600">
            ...
            <span className="font-medium text-stone-800">Developer</span>, or{" "}
            <span className="font-medium text-stone-800">Normal</span>. Normal users need approval
        ...
        <AdminUserRoleTable initialRows={initialRows} fetchError={error?.message ?? null} />
      </div>
    </div>
  );
}
